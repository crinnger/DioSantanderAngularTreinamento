...
export class CourseService {
  ...
  retrieveAllHttp(): Observable<Course[]> {
    return this.httpClient.get<Course[]>(this.coursesUrl);
  }

  getById(courseId: number): Course {
    return COURSES.find((course) => course.id === courseId) as Course;
  }

  getByIdHttp(courseId: number): Observable<Course> {
  ...
  save(course: Course): void {
    if (course.id) {
      const index = COURSES.findIndex(
        (courseInterator) => courseInterator.id === course.id
      );
  ...
  deleteByIdHttp(courseId: number): Observable<any> {
    return this.httpClient.delete<any>(`${this.coursesUrl}/${courseId}`);
  }
}
